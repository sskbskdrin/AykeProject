 ...
 * @date 6/9/12 4:41 PM
 */
public abstract class ExpandBaseAdapter extends BaseAdapter {
	/**
	 ...
	 */
	public abstract int[] getExpandToggleViewsID();
	...
	public void enableFor(View parent, int position) {
		if (parent == null)
			return;
		int[] views = getExpandToggleViewsID();
		List<View> toggles = new ArrayList<View>();
		for (int i : views) {
			toggles.add(parent.findViewById(i));
		}
		View itemToolbar = parent.findViewById(getExpandViewId());
		itemToolbar.measure(parent.getWidth(), parent.getHeight());

		enableFor(toggles, itemToolbar, position);
		itemToolbar.requestLayout();
	}

	private void enableFor(List<View> toggles, final View target,
			final int position) {
